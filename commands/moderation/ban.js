...
module.exports = {
    name: 'Ban',
    syntax: 'ban <@user|@role> [@user|@role]...',
    args: true,
    description: 'Entfernt die unwürdigen Wesen von deinem Server, FÜR IMMER ^^',
    perm: 'BAN_MEMBERS',
    commands: ['ban'],
    ...
    async execute(msg) {
        ...
        if (not_kicked.length > 0) {
            emb.addField(`**${not_kicked.length} Nutzer wurden nicht gebannt**`, not_kicked.join(', '));
        }

        if (kicked.length > 0) {
            emb.addField(`**Erfolreich gebannt wurden ${kicked.length} Nutzer**`, kicked.join(', '));
        }
